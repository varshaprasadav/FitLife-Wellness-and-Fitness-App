...
admin.post("/plans", authenticate, admincheck, upload.single("image"), async (req, res) => {
  try {
    const { title, price, bg, buttonColor, buttonText } = req.body;
    ...
    const daysArray = [
      ...assignDays(easyExercises,  10),
      ...assignDays(interExercises, 10),
      ...assignDays(hardExercises,  10),
    ].map((exercises, i) => ({ day: i + 1, exercises }));

    const newPlan = new Plan({ title, price, days: daysArray, bg, image, buttonColor, buttonText });
    await newPlan.save();

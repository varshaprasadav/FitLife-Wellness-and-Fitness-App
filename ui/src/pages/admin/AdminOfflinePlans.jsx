...
const AdminOfflinePlans = () => {
  ...
  const fetchPlans = async () => {
    setLoading(true);
    try {
      const res = await fetch("http://localhost:5000/api/offline-plans");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/offline-plans", {
        ...
        body: JSON.stringify({ ...form, features: featuresList }),
      });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/offline-plans/${id}`, {
        method: "DELETE",
      });
      if (res.ok) {
